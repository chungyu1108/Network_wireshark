	...
	OUT int *dsuccesses )
{
	...
	// calculate lengths of the distances between the centers of the pins and the centers of the holes:
	float lenA = Length( dholeaxs[gid]-PinAx, dholeays[gid]-PinAy );
	float lenB = Length( dholebxs[gid]-PinBx, dholebys[gid]-PinBy );
	float lenC = Length( dholecxs[gid]-PinCx, dholecys[gid]-PinCy );

	// if the length of the distance is smaller than the radius of the hole minus the radius of the pin, the pin fits the hole:
	if( lenA <= dholears[gid]-PinAr )
	{
		if( lenB <= dholebrs[gid]-PinBr )
		{
			if( lenC <= dholecrs[gid]-PinCr )
        			dsuccesses[gid] = 1;
		}
	}
}
